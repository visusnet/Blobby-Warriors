package
{
	...
	public class BGroundLine
	{
		...
		public function add()
		{
			...
			//parent.level.groundBody.CreateShape(boxDef);
		...
		function makeLine(p1:Point, p2:Point) : b2PolygonDef
		{
			...
			var length = Math.sqrt((xOffset * xOffset) + (yOffset * yOffset)) * 0.5;
			...
			//bodyDef.position.y += (5/parent.config.phy_scaling) * Math.cos(angle*(180/Math.PI));
			
			//bodyDef.position.y += (10/parent.config.phy_scaling) * Math.cos(angle*(180/Math.PI) % 360);
			//bodyDef.position.x += (10/parent.config.phy_scaling) * Math.sin(angle*(180/Math.PI) % 360);
			
			//bodyDef.position.y += 2.5*Math.cos(angle);
		...
		/*function makeLine(p1:Point, p2:Point) : b2PolygonDef
		{
			...
			var normal = new b2Vec2(p1.y - p2.y, p1.x - p2.x);
		   	...
		   	polygonDef.vertices[0] = new b2Vec2(p1.x, p1.y) + normal;
		   	polygonDef.vertices[1] = new b2Vec2(p1.x, p1.y);
		   	polygonDef.vertices[2] = new b2Vec2(p2.x, p2.y);
			polygonDef.vertices[3] = new b2Vec2(p2.x, p2.y) + normal;
			 ...
			 polygonDef.vertices[2].x = p2.x + normal.x;
			 polygonDef.vertices[2].y = p2.y + normal.y;
			 polygonDef.vertices[3].x = p1.x + normal.x;
			 polygonDef.vertices[3].y = p1.y + normal.y;
